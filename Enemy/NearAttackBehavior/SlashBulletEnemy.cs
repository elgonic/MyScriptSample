...
///弾発射
/// </summary>
public class SlashBulletEnemy : EnemyBehaviorBase
{
    [SerializeField] private NearAttackBase _attack;
    [SerializeField] private DropDownBullet _dropAttack;
    ...
    private IEnumerator Behavior()
    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                OnAttack?.Invoke();
                Coroutine dropAttack = _dropAttack.Attack(_attacker, _target);

                yield return dropAttack;

                yield return StaticCommonParams.Yielders.Get(3f);
            }
            yield return null;
        }
    }
